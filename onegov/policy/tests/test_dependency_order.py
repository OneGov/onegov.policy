...
class TestDependencyOrder(TestCase):
    ...
    def test_dependency_order_by_topological_sort(self):
        self.maxDiff = None

        all_profileids_topological = self.get_sorted_profile_ids()
        needed_profile_ids = self.get_default_profile_dependencies('default')
        ...
        try:
            self.assertEquals(
                ...
                'onegov.policy:default profiles are not in'
                ' topological order')
